...
output_filename = '/home/pi/Downloads/output_video.avi'  # 절대 경로로 변경
fourcc = cv2.VideoWriter_fourcc(*'XVID')
fps = 20.0  # Frames per second
frame_size = (640, 480)  # Resolution of the video frames (should match your camera resolution)
out = cv2.VideoWriter(output_filename, fourcc, fps, frame_size)

if not out.isOpened():
    rospy.logerr("Failed to open VideoWriter")
...
def hough_transform(edges, image):
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, threshold=30, minLineLength=30, maxLineGap=80)
...
def detect_edges(image):
    # 윗부분 삭제
    height, width = image.shape[:2]
    black_region_height = int(height * 0.7)  # 상위 70%를 검정색으로
    ...
    # 침식
    dilated_mask = cv2.dilate(combined_mask, kernel, iterations=4)
...
def main():
    ...
    while not rospy.is_shutdown():
        if cv_image.size != 0:
            ...
            cv_image_resized = cv2.resize(cv_image, frame_size)  # Resize frame to match the video resolution
            cv2.imshow("Steering Frame", line_image)
            out.write(cv_image_resized)  # Save the original frame to the video file
